...
Claude Code / Claude Desktop / Codex CLI から tsumugi を操作するための MCP サーバー。
...
このリポジトリの [`tsumugi_mcp.py`](tsumugi_mcp.py) をダウンロードし、任意の場所に保存する。
...
以下の例では `<保存先>` を、`tsumugi_mcp.py` を保存した実際のパスに置き換えること。
...
macOS / Linux:
```bash
claude mcp add tsumugi --scope user -- python3 <保存先>/tsumugi_mcp.py
```

Windows:
```bash
claude mcp add tsumugi --scope user -- python <保存先>\tsumugi_mcp.py
```
...
削除する場合:
```bash
claude mcp remove tsumugi -s user
```
...
macOS / Linux:
```bash
codex mcp add tsumugi -- python3 <保存先>/tsumugi_mcp.py
```

Windows:
```bash
codex mcp add tsumugi -- python <保存先>\tsumugi_mcp.py
```
...
args = ["<保存先>/tsumugi_mcp.py"]
```

以上で準備完了。AI エージェントから tsumugi のツールが使えるようになる。
tsumugi が起動していなくても `launch` ツールで MCP 経由でウィンドウを開ける。
...
```
Claude Code / Claude Desktop / Codex CLI → MCP(stdio) → Python → HTTP API → tsumugi
```

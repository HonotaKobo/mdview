...
## 概要
tsumugiはMCPサーバー経由でMarkdownをGUIウィンドウに表示・編集できるツール。
ターミナルやチャット欄で表示するには長すぎるMarkdownをtsumugiで出力する。

## 判断基準
ユーザーに見せるためのMarkdown出力（設計書、レポート、調査結果のまとめなど）はtsumugiで表示する。ターミナル上で簡潔に返せる短い回答にはtsumugiを使わない。

- 20行以上の構造化されたMarkdown出力はtsumugiを使う
- 表・リスト・見出しを多用する内容は行数に関わらずtsumugiを使う
...
`launch` が失敗した場合はアプリが起動していないため、ユーザーにtsumugiの起動を依頼すること。
